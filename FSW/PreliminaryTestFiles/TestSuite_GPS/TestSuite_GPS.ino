...
#define GPSSerial Serial1  // Pin 0 in, pin 1 out on Serial1

// Connect to the GPS on the hardware port
Adafruit_GPS GPS(&GPSSerial);

// Shouldn't be defined... I think
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Adafruit GPS library basic test. Code is in setup");
...
void loop() {
  ...
  if (GPS.newNMEAreceived()) {
    ...
    if (!GPS.parse(GPS.lastNMEA())){ // this also sets the newNMEAreceived() flag // to false
      Serial.println("DEBUG: Sentence parse failed");
      return; // we can fail to parse a sentence in which case we should just
              // wait for another
    }
  }

  // approximately every 2 seconds or so, random intervals, print GPS information
  ...
  if (millis() - timer > nextInterval) {
    ...
    if (GPS.fix) {
      ...
      Serial.println("DEBUG: GPS_NOFIX");
    }
